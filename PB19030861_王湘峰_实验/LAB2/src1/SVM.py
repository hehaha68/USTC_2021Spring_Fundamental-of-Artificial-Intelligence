...
class SupportVectorMachine:
    ...
    def fit(self, train_data, train_label, test_data):
        ...
        A = cvxopt.matrix(train_label, (1, train_num), 'd')     #转化为浮点数，否则会报错
        b = cvxopt.matrix(0.0)
        ...
        if self.C is None:  #是否软间隔
            ...
            G = cvxopt.matrix(np.vstack([M1, M2]))      #0 < alpha <C 软间隔
        ...
        a = np.ravel(solve['x'])        #得到所有的alpha
        sv = a > self.Epsilon           #得到支持向量的下标
        index = np.arange(len(a))[sv]
        alpha = a[sv]                   #支持向量的拉格朗日乘子
        SV = train_data[sv]             #支持向量
        SV_label = train_label[sv]      #支持向量的标签
        #B是偏置项
        B = 0
        ...
        elif self.kernel == 'Gauss' or self.kernel == 'Poly':   #非线性核时按照最初表达式计算
