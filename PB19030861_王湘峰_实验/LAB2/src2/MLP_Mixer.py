...
class MLPMixer(nn.Module):
    def __init__(self, patch_size, hidden_dim, depth):
        ...
        self.embed = nn.Conv2d(1, 14, kernel_size=patch_size, stride=patch_size)    #用卷积进行投射到14维向量上
        MIX = Mixer_Layer(patch_size, hidden_dim)
        self.mixlayers = nn.Sequential(*[MIX for _ in range(depth)])        #进行n = depth 次神经网络训练
    ...
    def forward(self, data):
        ...
        x = self.mixlayers(x)           #进行n = depth 次神经网络训练
        x = self.norm(x)
        x = torch.mean(x, dim=1)    #池化，global average pooling
        x = self.connect(x)     #全连接，fully connected
        return x
        ########################################################################
...
def train(model, train_loader, optimizer, n_epochs, criterion):
    ...
    for epoch in range(n_epochs):
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            pre = model(data)
            loss = criterion(pre, target)/len(data)
            loss.backward()
            optimizer.step()
            ########################################################################
            if batch_idx % 100 == 0:
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adamax(params, lr=10*learning_rate, betas=(0.9, 0.999), eps=1e-07, weight_decay=0)  #多次调参优化得到
    ########################################################################
